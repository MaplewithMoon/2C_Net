...
class CodedVision(nn.Module):
    def __init__(self,N,M):
        ...
        self.classifier = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=21)
        
    def forward(self, x, Training=True):
        encoded_raw, encoded = self.Encoder(x, training=Training)
        output = self.Decoder(encoded)
        encoded_hyper = self.Hypercoder.encoder(encoded_raw, training=Training)
        ...
        return output, likelihoods, loc, scale, cls
