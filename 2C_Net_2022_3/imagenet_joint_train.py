...
parser.add_argument('-data',default='/data1/imagenet/' , metavar='DIR', help='path to dataset')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    pretrained_dict = torch.load('./model/reducepool_2c_fixed_mid_epoch_63_acc_61.809998_state_dict.pth')
    ...
    criterion_rec = torch_msssim.MS_SSIM(max_val=1).cuda(args.gpu)
    '''
    ...
    optimizer = torch.optim.Adam([{'params':model.module.Encoder.parameters(),'lr':args.lr},
                                  {'params':model.module.Decoder.parameters(),'lr':args.lr},
                                  {'params':model.module.Hypercoder.parameters(),'lr':args.lr},
                                  {'params':model.module.estimator.parameters(),'lr':args.lr},
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        rec,rate = train(train_loader, model, criterion,criterion_rec, optimizer, epoch, args)
        
        # evaluate on validation set
        acc1 = validate(val_loader, model, criterion, args)
        torch.save(model.state_dict(),'./model/reducepool_2c_10-1-10_epoch_'+ str(epoch) + '_acc_' + str(acc1.data.cpu().numpy())+ '_rate_'+ str(rate) + '_msssim_' +str(1.-rec)+'_state_dict' +'.pth')
...
def train(train_loader, model, criterion, criterion_rec, optimizer, epoch, args):
    ...
    losses_rate = AverageMeter('Loss_rate', ':.5f')
    losses = AverageMeter('Loss', ':.4f')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    progress = ProgressMeter(
        len(train_loader),
        [ losses,losses_cls,losses_rec, losses_rate, top1,top5],
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        images = images.cuda(args.gpu, non_blocking=True)
        # compute output
        output,cls_output,bpp = model(images,Training=True)
        loss_cls = criterion(cls_output, target)
        loss_rec = 1.0-criterion_rec(output,images)
        loss_rate = bpp.mean()
        #print(loss_rate)
        # measure accuracy and record loss
        loss = 10.0*loss_cls + 1.0*loss_rate + 10.0*loss_rec
        ...
        losses_rate.update(loss_rate.item(), images.size(0))
        ...
        if i % args.print_freq == 0:
            progress.display(i)
    return losses_rec.avg, losses_rate.avg
