...
class Deepcoder_encoder(nn.Module):
    def __init__(self, N=192, M=320):
        ...
        self.gdn0 = gdn_layer.GDN(ch=num_channels,device=torch.cuda.current_device())
        
        self.conv_b1 = res_block(channels=num_channels)
        self.conv1 = nn.Conv2d(num_channels, num_channels, (5, 5), stride=(2, 2), padding=2, bias=True)
        self.gdn1 = gdn_layer.GDN(ch=num_channels,device=torch.cuda.current_device())
        
        self.conv_b2 = res_block(channels=num_channels)
        self.conv2 = nn.Conv2d(num_channels, num_channels, (5, 5), stride=(2, 2), padding=2, bias=True)
        self.gdn2 = gdn_layer.GDN(ch=num_channels,device=torch.cuda.current_device())
...
class Deepcoder_decoder(nn.Module):
    def __init__(self, N=192, M=320):
        ...
        self.conv_b4 = res_block(channels=num_channels)
        self.igdn0 = gdn_layer.GDN(ch=num_channels,device=torch.cuda.current_device())
        
        self.up1 = nn.ConvTranspose2d(num_channels, num_channels, (5, 5), stride=(2, 2), padding=2, output_padding=1)
        self.conv_b5 = res_block(channels=num_channels)
        self.igdn1 = gdn_layer.GDN(ch=num_channels,device=torch.cuda.current_device())
        ...
        self.igdn2 = gdn_layer.GDN(ch=num_channels,device=torch.cuda.current_device())
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer2 = self._make_layer(block, 128,512, layers[1], stride=1)
        self.layer3 = self._make_layer(block, 512,512, layers[2], stride=1)
        self.layer4 = self._make_layer(block, 512,2048, layers[3], stride=2)
...
class CodedVision(nn.Module):
    def __init__(self,N,M):
        ...
        self.classifier = ResNet(Bottleneck, [3, 4, 6, 3], num_classes=1000)
